# Graph Report - project_random  (2026-08-14)

## Corpus Check
- 118 files · ~229,338 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 688 nodes · 1196 edges · 71 communities (49 shown, 22 thin omitted)
...
1. `verifySession` - 61 edges
2. `db` - 18 edges
3. `compilerOptions` - 17 edges
4. `hotelDate()` - 16 edges
...
- `ImportGuestsPage()` --calls--> `verifySession`  [EXTRACTED]
  src/app/dashboard/guests/import/page.tsx → src/lib/dal.ts
- `DashboardLayout()` --calls--> `verifySession`  [EXTRACTED]
  src/app/dashboard/layout.tsx → src/lib/dal.ts
- `CalendarPage()` --calls--> `hotelDate()`  [EXTRACTED]
  src/app/dashboard/calendar/page.tsx → src/lib/hotel-date.ts
- `CalendarPage()` --calls--> `addCalendarDays()`  [EXTRACTED]
  src/app/dashboard/calendar/page.tsx → src/modules/calendar/date-window.ts
...
Cohesion: 0.06
Nodes (70): bulkSchema, bulkUpdateRoomsAction(), createLocationAction(), createRoomAction(), createRoomTypeAction(), createStaffAction(), deleteLocationAction(), deleteRoomAction() (+62 more)

### Community 1 - "Guest Management Actions and Forms"
Cohesion: 0.15
Nodes (17): createGuestAction(), duplicateState(), GuestActionState, invalidInput(), updateGuestAction(), GuestDefaults, GuestForm(), GuestProfilePage() (+9 more)

### Community 2 - "Housekeeping Status Management and Board"
Cohesion: 0.06
Nodes (46): bedIdSchema, bedStatusSchema, RoomStatusActionState, updateRoomStatusAction(), filters, groupByFloor(), HousekeepingPage(), RoomStatusControl() (+38 more)

### Community 3 - "Reservation Lifecycle and Validation Rules"
Cohesion: 0.21
Nodes (8): ReservationActionState, ReservationDetailPage(), LifecycleActions(), Status, StatusBadge(), ReservationStatus, getReservation(), getReservationOptions()

### Community 4 - "Interactive Calendar and Date Timeline Grid"
Cohesion: 0.19
Nodes (16): CalendarControls(), RoomStatus, first(), NewReservationPage(), NewReservationParams, RoomsControls(), RoomsControlsProps, addCalendarDays() (+8 more)
...
Cohesion: 0.19
Nodes (10): DashboardLayout(), logout(), AppShell(), roleLabel(), Navigation(), formatHotelDay(), can(), Permission (+2 more)
...
Cohesion: 0.21
Nodes (13): checkGuestDuplicatesAction(), DuplicateCheckResult, importGuestsAction(), fieldLabels, ImportForm(), ParsedRow, ImportGuestsPage(), mapHeaders() (+5 more)
...
Cohesion: 0.21
Nodes (12): bedStatusLabels, reservationStatusLabels, RoomsPageProps, RoomsStatusPage(), getRoomsStatusData(), BedData, filterGroupedRooms(), groupRooms() (+4 more)

### Community 62 - "status-transition.ts"
Cohesion: 0.39
Nodes (4): authenticate(), LoginState, initialState, LoginForm()

### Community 64 - "Design: Bulk Bed Generation (Slice 8)"
Cohesion: 0.22
Nodes (8): 1. Understanding Summary, 2. Assumptions & NFRs, 3. Decision Log, 4. Proposed Implementation Detail, Design: Bulk Bed Generation (Slice 8), Purpose, Server Action, Zod Validation

### Community 65 - "page.tsx"
Cohesion: 0.27
Nodes (11): CalendarPage(), CalendarStyle, dateIndex(), dayLabel(), groupBy(), guestLabel(), longDayLabel(), nightCount() (+3 more)

### Community 66 - "actions.ts"
Cohesion: 0.33
Nodes (11): createReservationAction(), databaseMessage(), invalidInput(), searchGuestsAction(), updateReservationAction(), getRecentGuests(), searchGuests(), findConflictingGenderReservation() (+3 more)

### Community 67 - "validation.ts"
Cohesion: 0.22
Nodes (9): createReservationSchema, isoDate, ReservationInput, reservationInputSchema, dayAfter, pastDate, today, tomorrow (+1 more)

### Community 68 - "reservation-form.tsx"
Cohesion: 0.22
Nodes (7): GuestCombobox(), GuestComboboxProps, GuestOption, Defaults, OptionData, ReservationForm(), statusLabels

### Community 69 - "hotel-date.ts"
Cohesion: 0.54
Nodes (5): DashboardPage(), dateParts(), formatShortDate(), hotelDate(), getDashboardData()

### Community 70 - "transitionReservationAction"
Cohesion: 0.47
Nodes (4): transitionReservationAction(), allowedTransitions, canTransition(), ReservationStatus

## Knowledge Gaps
- **299 isolated node(s):** `nextConfig`, `name`, `version`, `type`, `private` (+294 more)
...
- **Why does `verifySession` connect `Settings Actions and Admin Configurations` to `Guest Management Actions and Forms`, `Housekeeping Status Management and Board`, `actions.ts`, `page.tsx`, `hotel-date.ts`, `transitionReservationAction`, `Reservation Lifecycle and Validation Rules`, `Authenticated Layout Shell and Navigation`, `queries.ts`, `validation.ts`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `hotelDate()` connect `hotel-date.ts` to `page.tsx`, `actions.ts`, `validation.ts`, `Interactive Calendar and Date Timeline Grid`, `transitionReservationAction`, `validation.ts`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `db` connect `Housekeeping Status Management and Board` to `Settings Actions and Admin Configurations`, `Guest Management Actions and Forms`, `actions.ts`, `hotel-date.ts`, `queries.ts`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
