...
export function GuestForm({ action, defaults = {}, submitLabel, cancelHref }: {
  ...
  return <form action={formAction} className={styles.form}>
    {state.message ? <div className={styles.formMessage} data-status={state.status} role="alert">{state.message}{state.duplicateId ? <> <Link href={`/dashboard/guests/${state.duplicateId}`}>Lihat tamu yang ada</Link></> : null}</div> : null}
    ...
    <div className={styles.formActions}><Link className={styles.secondaryButton} href={cancelHref}>Batal</Link><SubmitButton label={submitLabel} /></div>
  </form>;
}
